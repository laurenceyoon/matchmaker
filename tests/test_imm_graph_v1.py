...
from matchmaker.prob.imm_graph_v1 import ZV, IMMGraphFollowerV1 as IMMGraphFollower
...
def test_repeat_jump_splits_advance_by_graph_prior(monkeypatch):
    ...
    # an overdue hypothesis may skip ahead on the linear path; the graph prior splits the mass
    assert follower.p[follower.k == 0].sum() == pytest.approx(follower.p[follower.k > 7].sum())
...
def test_example_piece_runs_to_the_end_without_backward_jumps():
    ...
    mm = Matchmaker(score_file=piece["score"], performance_file=piece["audio"],
                    input_type="audio", method="imm_graph_v1", wait=False)
