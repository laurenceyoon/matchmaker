...
class ScoreDirectedGraphBuilder:
    def __init__(self, linear_prior: float = 0.8, jump_prior: float = 0.2) -> None:
        self.linear_prior = linear_prior
        self.jump_prior = jump_prior

    def build(self, score: str | Path | pt.score.Score | pt.score.Part) -> ScoreDirectedGraph:
        ...
        for idx, m in enumerate(measures):
            ...
            meta = {"measure_index": idx, "measure_number": m.number, "duration_beats": end_b - start_b}
        ...
        for src, tgt in zip(measure_nodes, measure_nodes[1:]):
            graph.add_edge(ScoreEdge(src, tgt, EdgeKind.LINEAR, prior=self.linear_prior))
        ...
        for r in part.iter_all(pt.score.Repeat):
            if r.end is not None:
                ...
                graph.add_edge(ScoreEdge(measure_nodes[src], measure_nodes[tgt], EdgeKind.REPEAT, prior=self.jump_prior))
        ...
        if len(endings) >= 2:
            ...
            if src >= 0 and tgt < len(measure_nodes):
                graph.add_edge(ScoreEdge(measure_nodes[src], measure_nodes[tgt], EdgeKind.VOLTA, prior=self.jump_prior))

        for dc in part.iter_all(pt.score.DaCapo):
            src = ending_at(dc.start.t)
            graph.add_edge(ScoreEdge(measure_nodes[src], measure_nodes[0], EdgeKind.DA_CAPO, prior=self.jump_prior))
        ...
        if segnos:
            for ds in part.iter_all(pt.score.DalSegno):
                ...
                graph.add_edge(ScoreEdge(measure_nodes[src], measure_nodes[tgt], EdgeKind.DAL_SEGNO, prior=self.jump_prior))
        ...
        if codas:
            for tc in part.iter_all(pt.score.ToCoda):
                ...
                graph.add_edge(ScoreEdge(measure_nodes[src], measure_nodes[tgt], EdgeKind.CODA, prior=self.jump_prior))
...
def rebase_measure_beats(
    ...
    *,
    ...
    for i, nid in enumerate(graph.measure_nodes):
        old = graph.nodes[nid]
        dur = (starts[i + 1] - starts[i]) if i + 1 < len(starts) else (
            (final_beat - starts[i]) if final_beat is not None else float(old.metadata.get("duration_beats", 0.0))
        )
        meta = dict(old.metadata, duration_beats=dur)
        new_graph.add_node(ScoreNode(old.node_id, starts[i], old.measure, old.event_ids, metadata=meta))
