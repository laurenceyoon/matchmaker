...
def estimate_chord_position(
    ...
    ``hold_mode``, if given, is the column of a mode (e.g. ZV) that holds the
    chord rather than progressing through it by tempo; its elapsed-age can grow
    arbitrarily large while sustained, so its progress fraction is fixed at 0
    (the chord's onset) instead of being read off elapsed time over duration.
    """
